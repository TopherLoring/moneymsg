...
- Minor wording normalization applied during repository import

---

# Milestone 11 — Android IME Build

**Core principle:** MoneyMsg is a specialized payment and action IME panel, not a full replacement keyboard. It lives in the keyboard system for instant conversation access. It does not compete with Gboard on typing quality.
...
- [ ] Add clarification UX for unresolved recipient or missing amount
  - **Notes:** Tiny follow-ups for amount and recipient clarification instead of full-form entry.

- [ ] Add recipient suggestion row under the amount/recipient section
  - **Notes:** Show recent contacts, frequent counterparties, and thread-aware suggestions with confidence-tiered display.

- [ ] Add intent detection modes
  - **Notes:** Detect mode, request mode, split mode, reminder mode, and repeat mode for recurring patterns such as rent, utilities, rides, dinner, and freelance flows.
...
- [ ] Add fast cached balance display
  - **Notes:** Show last-known balance instantly. Refresh in background with short-lived token. Show stale-state indicator if needed. Do not make keyboard depend on slow live fetch every open.
...
- [ ] Add insufficient-balance behavior
  - **Notes:** Suggest Add Money and funded send.
...
- [ ] Build insertion renderer for chat artifacts
  - **Notes:** Insert polished text/card payload, not raw URL. Use display text that looks like a payment action.

- [ ] Add sender-side confirmation view before insertion

- [ ] Add thread follow-up message generation
  - **Notes:** After completion, offer generated in-thread responses for major state changes.

- [ ] Add graceful degradation
  - **Notes:** Rich object where possible. Formatted text plus hidden action path where the chat app only supports plain text.
