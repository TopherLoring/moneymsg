...
1. Structured data cleanup — convert riskMeta, deviceInfo, and payload fields to queryable storage (2.1.1) and add DB-backed request idempotency indexes (2.1.2).
2. Expand funding-source lifecycle states (2.1.3) to unblock wallet UX.
3. Add wallet summary, preview, and listing endpoints (3.1.1–3.1.3) and deep-link helpers (3.2.1).
...
| 19 | [x] | 1.1.1 | P0 | Fix | Completed | 0.1.1,0.4.2 | Risk | Replace the placeholder risk scorer with a real decision engine. | src/domain/risk/scorer.ts | Risk engine returns allow, step_up, review, or deny with reason codes. | scorer.ts now evaluates amount hard limits, device signals (vpn, jailbreak, newDevice, deviceChange), IP absence, riskMeta flags (velocityFlag, ipReputation), and amount thresholds into a 0–100 score mapped to allow/step_up/review/deny. assertRiskAllow throws RISK_DENY, RISK_REVIEW, or RISK_STEP_UP with typed error codes. | Placeholder removed. |
| 20 | [x] | 1.1.2 | P0 | Refactor | Completed | 1.1.1 | Risk | Expand hard-rule checks for velocity, duplicates, first-use, anomaly, and scam signals. | src/domain/risk/index.ts; src/domain/risk/signals.ts | Route-level risk checks cover users, devices, IPs, recipients, and funding sources. | signals.ts created with isFirstUse, isAmountAnomaly, hasRecentFailure, and exceedsVelocity query helpers. | Production fraud baseline. |
| 21 | [x] | 1.1.3 | P0 | Fix | Completed | 1.1.2 | Validation | Replace open-ended device and risk schemas with explicit typed contracts. | src/shared/schemas.ts | Client payloads follow bounded schemas and reject garbage or overcollection. | deviceInfoSchema and riskMetaSchema now use additionalProperties:false with explicit typed properties; all nullable flags are enumerated. | Improves privacy and analytics quality. |
| 22 | [x] | 1.1.4 | P0 | Add | Completed | 1.1.3,0.1.2 | Validation | Add strong KYC schema validation. | src/modules/kyc/schemas.ts; src/modules/kyc/http/routes.ts | KYC requests receive field-level validation before provider submission. | kycDataSchema created with required firstName, lastName, dateOfBirth (YYYY-MM-DD), ssn (9-digit), and structured address object; wired into route schema. | Support and compliance friendly. |
| 23 | [x] | 1.1.5 | P0 | Fix | Completed | 1.1.3,0.1.3 | Validation | Add strong Plaid payload validation and duplicate funding-source prevention. | src/modules/plaid/http/routes.ts | Plaid exchange validates shape and cannot create duplicate linked sources. | Exchange route validates publicToken minLength, accountId minLength, institutionName maxLength, and mask 4-digit pattern. Duplicate check added: rejects accountId already linked via plaidItemId. | Ownership plus shape validation. |
| 24 | [x] | 1.2.2 | P0 | Refactor | Completed | 1.2.1 | Webhooks | Expand the reconciliation state model and add replay-safe reconciliation. | src/modules/webhooks/http/routes.ts; src/modules/reconciliation/service.ts; src/infrastructure/db/schema.ts; drizzle/0010_reconciliation_state.sql | Webhook events support logged, reconciled, and finalized distinctions with replay-safe processing. | webhookStateEnum and new columns (reconciliationState, correlationRequestId, reconciledAt, finalizedAt, retryCount, lastError) added to schema. reconciliation/service.ts implements the logged→reconciled→finalized state machine. Webhook routes delegate to reconcileWebhookEvent + finalizeWebhookEvent. Migration 0010 created. | Core settlement hardening. |
| 25 | [x] | 1.2.3 | P1 | Add | Completed | 1.2.2 | Workers | Add a reconciliation sweeper and retry worker. | src/jobs/reconciliationSweeper.ts | Retryable or unresolved reconciliation work is processed safely. | reconciliationSweeper.ts sweeps events stuck in logged/reconciled state after 30 minutes, retries up to 5 times, finalizes reconciled events, and logs errors. Registered in server.ts. | Improves resilience. |
| 26 | [x] | 1.3.1 | P1 | Refactor | Completed | 0.4.2 | Providers | Normalize provider error classes and attach correlation metadata. | src/integrations/alviere/client.ts; src/integrations/alviere/kyc.ts; src/integrations/plaid/client.ts; src/integrations/tabapay/client.ts; src/integrations/dwolla/client.ts; src/shared/errors.ts | Provider failures map to internal enums and carry correlation IDs for support and audit. | ProviderError class added to errors.ts with provider name, providerStatus, and correlationId. All five provider call sites updated to throw ProviderError. | Ops-ready provider layer. |
