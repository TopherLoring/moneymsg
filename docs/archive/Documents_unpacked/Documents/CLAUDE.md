...
- **All fee calculations go through `src/domain/fees/index.ts`.** The platform fee is `1.75% + $0.50` on all external money movement. Never inline this calculation.
...
- Intents expire after 24 hours. The sweeper worker (`src/jobs/intentSweeper.ts`) reverses held funds on expiry.
