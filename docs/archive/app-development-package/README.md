...
## Risk hooks
- Daily limits and duplicate window configured in `src/config/constants.ts`.
- Risk metadata (device/ip/riskMeta) is stored per transaction and passed through a pluggable scorer in `src/domain/risk/scorer.ts`.
